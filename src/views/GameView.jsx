...
const GameView = ({
  ...
  // Logic to determine which unit is "Selected" (explicitly or by default)
  const defaultUnit = myBoard.find(u => u.canAttack && u.type !== 'support');
  const effectiveSelectedId = selectedUnitId || (defaultUnit ? defaultUnit.instanceId : null);

  const selectedUnit = effectiveSelectedId ? myBoard.find(u => u.instanceId === effectiveSelectedId) : null;
    ...
    <div className={`flex flex-col h-full bg-gray-950 overflow-hidden relative select-none ${isProcessing ? 'cursor-wait pointer-events-none grayscale-[0.3]' : ''}`}>
          ...
          {myBoard.map((u, i) => (
            <div key={i} className="relative group cursor-pointer hover:-translate-y-3 transition-all duration-200 min-w-max" onClick={() => handleBoardClick(u, i, false)}>
              <Card
                ...
                isDeployed={true}
                isSelected={effectiveSelectedId === u.instanceId}
      ...
      <div className="h-64 bg-gray-950 border-t-4 border-gray-800 flex relative z-30 shadow-2xl">
        ...
        <div className="flex-1 flex items-center space-x-4 overflow-x-auto p-2 pb-4">
          {myHand
            .map((cardId, i) => ({ id: cardId, originalIndex: i, data: CARD_DATABASE[cardId] }))
            .sort((a, b) => {
              // Sort Priority: Support > Tactic > Unit (Combat)
              const typePriority = { 'support': 1, 'tactic': 2 };
              const typeA = typePriority[a.data.type] || 3;
              const typeB = typePriority[b.data.type] || 3;

              if (typeA !== typeB) return typeA - typeB;
              return a.data.cost - b.data.cost; // Secondary sort by cost
            })
            .map((item, i) => (
              <div key={`${item.id}-${item.originalIndex}`} className="hover:-translate-y-2 transition-transform duration-300 z-10 min-w-max">
                <Card
                  cardId={item.id}
                  size="medium"
                  onClick={() => handlePlayCard(item.id, item.originalIndex)}
                  disabled={!isMyTurn || item.data.cost > myMana}
