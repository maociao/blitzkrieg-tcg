...
const CardArt = ({ type, id, artPrompt, customSeed, className = 'h-24' }) => {
  ...
  useEffect(() => {
    ...
    const loadArt = async () => {
      ...
      try {
        // 1. Try to get from Firebase Storage Cache (ONLY if no custom seed)
        // If custom seed exists, we assume it's not cached yet or we want to use the generator directly to avoid 404s.
        if (!customSeed) {
          ...
          // Direct fallback for custom seeds
          throw new Error("Custom seed - skipping storage check");
        }
      } catch (error) {
