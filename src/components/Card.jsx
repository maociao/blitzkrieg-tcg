    ...
    <div className={`${baseClass} ${sizeClass}`} onClick={!disabled ? onClick : undefined}>
      ...
      <div className={`relative w-full h-full transition-transform duration-500 [transform-style:preserve-3d] ${!isDeployed && allowFlip ? 'group-hover:[transform:rotateY(180deg)]' : ''}`}>
        ...
        <div className={`absolute inset-0 flex flex-col rounded-lg border-2 p-2 bg-gray-900 backface-hidden ${RARITY_COLORS[data.rarity]}`}>
          {/* Tactic Description Overlay */}
          {data.type === 'tactic' && (
            <div className="absolute bottom-1 left-1 right-1 bg-black/70 text-white text-[10px] p-1 rounded text-center border border-gray-600 leading-tight z-20">
              {data.desc}
            </div>
          )}
          ...
          {size === 'large' ? (
            <div className={`w-full flex-1 rounded-md overflow-hidden relative bg-gray-700 min-h-0 ${data.type === 'tactic' ? 'mb-8' : 'mb-1'}`}>
              {isLoading && !hasError && (
            ...
            <CardArt type={data.type} id={cardId} artPrompt={data.artPrompt} customSeed={customSeed || (artOverrides ? artOverrides[cardId] : null)} className={`flex-1 min-h-0 ${data.type === 'tactic' ? 'mb-8' : 'mb-1'}`} />
          )}
